...
app.post("/battle/start", authenticateToken, async (req, res) => {
  try {
    ...
    if (user.stats.health <= 10) {
      return res.status(400).json({ success: false, message: "Your health is too low! Buy health potions first." });
    }
    ...
    res.status(500).json({ success: false, message: "Server error starting battle" });
  }
});
...
app.post("/battle/attack", authenticateToken, async (req, res) => {
  try {
    ...
    const activeBattle = user.battles.filter(battle => !battle.completed).pop();
    ...
    res.status(500).json({ success: false, message: "Server error in battle attack" });
  }
});
...
app.post("/shop/buy-health", authenticateToken, async (req, res) => {
  try {
    ...
    if (user.gold < cost) {
      return res.status(400).json({ success: false, message: `Not enough gold! Need ${cost} gold.` });
    }
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 FitQuest server running at http://localhost:${PORT}`);
